...
// header menu
if($(window).width() > 993) {
	$('.header__menus>li.menu-item-has-children')
	...
	$('.header__menus .first_level_menu>ul>li').hover(function() {
		if(!$(this).is(':first')) {
			$(this).parents('li').find('.first_level_menu>ul>li.menu-item:first').removeClass('active');
			$(this).toggleClass('active');
		}
	})
	$('.header__menus .first_level_menu>ul>li li').hover(function() {
		$(this).toggleClass('active');
	})
} else {
	$('.header__menus li').click(function(e) {
		e.stopPropagation();
		$(this).toggleClass('active');
	})
}
